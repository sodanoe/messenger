...
def setup(client):
    ...
    for name_key, token_key, uid_key in [
        ...
        reg = client.post("/auth/register", json={
            "username": name_key, "password": "password123", "invite_code": code,
        })
    ...
    # Alice ↔ Bob contacts
    r = client.post("/contacts", json={"username": f"s5_bob_{RUN}"},
                    headers=auth_headers(state["token_a"]))
    assert r.status_code == 201

    # Alice sends Bob a message
    r = client.post(f"/messages/{state['uid_b']}",
                    json={"content": "test message"},
                    headers=auth_headers(state["token_a"]))
    assert r.status_code == 201

    # Alice creates group, invites Bob
    g = client.post("/groups", json={"name": f"S5Group_{RUN}"},
                    headers=auth_headers(state["token_a"]))
    assert g.status_code == 201
    state["group_id"] = g.json()["id"]
    client.post(f"/groups/{state['group_id']}/invite",
...
def test_2_message_isolation(client):
    ...
    resp = client.get(f"/messages/{state['uid_b']}",
                      headers=auth_headers(state["token_c"]))
...
def test_3_blocked_user_cannot_message(client):
    # Alice блокирует Bob
    resp = client.post(f"/contacts/{state['uid_b']}/block",
                       headers=auth_headers(state["token_a"]))
    assert resp.status_code == 204

    # Bob пытается написать Alice
    resp2 = client.post(f"/messages/{state['uid_a']}",
...
def test_4_non_member_group_access_denied(client):
    resp = client.get(f"/groups/{state['group_id']}/messages",
                      headers=auth_headers(state["token_c"]))
    assert resp.status_code == 403, f"Expected 403, got {resp.status_code}"

    resp2 = client.get(f"/groups/{state['group_id']}/members",
                       headers=auth_headers(state["token_c"]))
...
def test_8_delete_account(client):
    ...
    client.post(f"/messages/{reg2.json().get('id', doomed_id)}",
                json={"content": "bye"},
                headers=auth_headers(doomed_token))
...
def test_9_cascade_wipe(client):
    ...
    contacts_after = client.get("/contacts", headers=auth_headers(state["token_b"])).json()
...
def test_10_deleted_user_not_in_search(client):
    ...
    search_before = client.get(f"/users/search?q={ghost_name}",
                                headers=auth_headers(state["token_a"])).json()
    ...
    search_after = client.get(f"/users/search?q={ghost_name}",
                               headers=auth_headers(state["token_a"])).json()
