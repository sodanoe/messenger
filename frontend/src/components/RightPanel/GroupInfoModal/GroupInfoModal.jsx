...
import {
  getGroupMembers, inviteMember, removeMember, leaveGroup, deleteGroup, getGroups,
...
export default function GroupInfoModal({ onClose }) {
  const { currentChat, me, contacts, setGroups, clearCurrentChat } = useAppStore()
    ...
    <div className={styles.overlay} onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className={styles.modal}>
          ...
          {members.map((m) => (
            <div key={m.user_id} className={styles.member}>
              ...
              {m.user_id === me?.id
                ...
                : isAdmin && (
                  <button className={styles.removeBtn} onClick={() => doRemove(m.user_id, m.username)}>✕</button>
                )
              }
            </div>
          ))}
        ...
        {available.length > 0 && (
          <>
            ...
            <div className={styles.pickerList}>
              {available.map((c) => (
                <button key={c.contact_user_id} className={styles.pickerItem} onClick={() => doInvite(c)}>
        ...
        <div className={styles.inviteRow}>
          ...
          />
          <button className={styles.inviteBtn} onClick={() => doInvite(inviteInput)}>+ Добавить</button>
        </div>

        <button className={styles.leaveBtn} onClick={doLeave}>Покинуть группу</button>
        {isAdmin && (
          <button className={styles.deleteBtn} onClick={doDelete}>🗑 Удалить группу</button>
        )}
      </div>
    </div>
  )
