...
function renderMessages(msgs) {
  ...
  msgs.forEach(m => {
    ...
    if (day !== lastDay) {
      ...
      wrap.appendChild(d); lastDay = day;
    }
    appendMessage({
      id: m.id,
      content: m.content,
...
function appendMessage({ id = null, content, isMe, createdAt, mediaUrl = null, replyTo = null, reactions = [] }, animate = true) {
  ...
  let replyHtml = '';
  if (replyTo) {
    const author = replyTo.sender_id === me?.id ? 'Вы' : (currentChat?.name || `#${replyTo.sender_id}`);
...
function scrollBottom(smooth = false) {
  const w = el('messages');
  requestAnimationFrame(() => w.scrollTo({ top: w.scrollHeight, behavior: smooth ? 'smooth' : 'instant' }));
}
