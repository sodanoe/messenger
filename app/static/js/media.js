...
function onDragOver(e) {
    ...
    if ([...(e.dataTransfer?.types || [])].includes('Files'))
        el('chat-input-area').classList.add('drag-over');
}
function onDragLeave(e) {
    if (el('chat-input-area').contains(e.relatedTarget)) return;
    el('chat-input-area').classList.remove('drag-over');
}
function onDrop(e) {
    e.preventDefault();
    el('chat-input-area').classList.remove('drag-over');
    ...
    if (f && f.type.startsWith('image/')) uploadMediaFile(f);
}

async function uploadMediaFile(file) {
    if (file.size > 20 * 1024 * 1024) {
        toast('Файл слишком большой (макс 20MB)', 'err');
        return;
    }
    const btn = el('attach-btn');
    btn.disabled = true;
    btn.textContent = '⏳';
    try {
        ...
        reader.onload = (e) => {
            const c = el('media-preview-container');
