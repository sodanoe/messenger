...
async function doLogin() {
  ...
  const btn = el('login-btn'), err = el('login-err');
...
async function doRegister() {
  const username = v('r-user'), password = v('r-pass'), invite_code = v('r-invite');
  if (!username || !password || !invite_code) return showErr(el('reg-err'), 'Заполни все поля');
  const btn = el('reg-btn'), err = el('reg-err');
  err.style.display = 'none'; btn.disabled = true; btn.textContent = '…';
  try {
    const r = await api('/auth/register', 'POST', { username, password, invite_code });
    await onAuthSuccess(r.access_token);
  } catch(e) { showErr(err, e.message); }
...
el('r-invite').addEventListener('keydown', e => { if (e.key === 'Enter') doRegister(); });
...
async function checkAdmin() {
  try { await api('/admin/media-settings'); return true; } catch { return false; }
}

function doLogout() {
  token = null; me = null; isAdmin = false; currentChat = null;
  lastInvite = null; pendingMediaId = null; replyTo = null;
  localStorage.removeItem('msng_token');
  wsClose(); clearSearch(); hideReactionPicker();
