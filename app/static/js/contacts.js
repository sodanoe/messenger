...
function onSearchInput(val) {
  ...
  if (!trimmed) { hideSearch(); return; }
  searchTimer = setTimeout(() => doSearch(trimmed), 280);
}

async function doSearch(q) {
  try { renderSearch(await api(`/users/search?q=${encodeURIComponent(q)}`)); }
  catch { renderSearch([]); }
}
...
function renderSearch(users) {
  ...
  searchActive = true;
  if (!users.length) { wrap.innerHTML = '<div class="search-empty">Никого не найдено</div>'; return; }
  const contactIds = new Set(contacts.map(c => c.contact_user_id));
  wrap.innerHTML = '<div class="search-header">Результаты поиска</div>' + users.map(u => {
...
function hideSearch()   { searchActive = false; el('search-results').style.display = 'none'; el('sidebar-tabs').style.display = 'flex'; el('contact-list').style.display = 'block'; }
function clearSearch()  { el('search-input').value = ''; el('search-clear').style.display = 'none'; clearTimeout(searchTimer); hideSearch(); }

async function addAndChat(userId, username) {
  try { await api('/contacts', 'POST', { username }); await loadContacts(); toast(`${username} добавлен`, 'ok'); } catch(e) { if (!e.message.includes('409') && !e.message.includes('already')) { toast(e.message, 'err'); return; } }
  clearSearch(); openDMById(userId, username, false);
}
// FIX: обёртки для data-атрибутов (защита от XSS через inline onclick + username)
function openDMFromEl(btn)    { const uid = +btn.dataset.uid, uname = btn.dataset.uname, online = btn.dataset.online === 'true'; clearSearch(); openDMById(uid, uname, online); }
function addAndChatFromEl(btn){ addAndChat(+btn.dataset.uid, btn.dataset.uname); }
async function openDMById(userId, username, isOnline) { clearSearch(); const c = contacts.find(c => c.contact_user_id === userId); openDM(userId, username, c ? c.is_online : isOnline); }

// ─────────────────────────────────────────────────────────
//  Contacts / Groups
// ─────────────────────────────────────────────────────────

async function loadContacts() { try { contacts = await api('/contacts'); if (activeTab === 'dm' && !searchActive) renderContacts(); } catch { toast('Ошибка загрузки контактов', 'err'); } }

function renderContacts() {
  const wrap = el('contact-list');
  if (!contacts.length) { wrap.innerHTML = '<div class="empty-list">Нет контактов.<br>Найди пользователя через поиск.</div>'; return; }
  wrap.innerHTML = contacts.map(c => `<div class="contact-item ${isActiveDM(c.contact_user_id)}" onclick="openDM(${c.contact_user_id},'${esc(c.username)}',${c.is_online})">
...
async function loadGroups() { try { groups = await api('/groups'); if (activeTab === 'groups' && !searchActive) renderGroups(); } catch { toast('Ошибка загрузки групп', 'err'); } }
