...
function renderGroupMembers(members) {
  ...
  wrap.innerHTML = members.map(m => {
    ...
    const removeBtn = isAdmin && !isMe
      ...
      : (isMe ? '<span style="font-size:11px;color:var(--text2);padding:0 4px">вы</span>' : '');
...
function renderContactPicker() {
  ...
  if (!available.length) {
    wrap.innerHTML = '<div style="color:var(--text2);font-size:12px;padding:4px 0">Все контакты уже в группе</div>';
    return;
  }

  wrap.innerHTML = available.map(c =>
